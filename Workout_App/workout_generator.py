...
class Workout:
    ...
    def __init__(self):
        ...
        self.recap_exos = json.load(
            open(dir + "\\recap_exos.json", "r", encoding="utf-8")
        )
        self.exos_par_zone = json.load(
            open(dir + "\\exos_par_zone.json", "r", encoding="utf-8")
        )
    ...
    def find_zones(self):
        ...
        nb_zones = self.nb_zones if self.nb_zones else 0
        zones_travail = [choice(zones) for i in range(nb_zones)]
        ...
        for exo in self.exos:
            if self.difficulty == "Difficile":
                if self.recap_exos[exo]["repetitions"][0] is None:
                    self.workout[exo] = (
                        str(self.recap_exos[exo]["duree (s)"][1]) + " secondes"
                    )
                else:
                    self.workout[exo] = (
                        str(self.recap_exos[exo]["repetitions"][1]) + " repetitions"
                    )

            elif self.difficulty == "Facile":
                if self.recap_exos[exo]["repetitions"][0] is None:
                    self.workout[exo] = (
                        str(self.recap_exos[exo]["duree (s)"][0]) + " secondes"
                    )
                else:
                    self.workout[exo] = (
                        str(self.recap_exos[exo]["repetitions"][0]) + " repetitions"
                    )
                ...
                if self.recap_exos[exo]["repetitions"][0] is None:
                    self.workout[exo] = (
                        str(
                            (
                                self.recap_exos[exo]["duree (s)"][0]
                                + self.recap_exos[exo]["duree (s)"][1]
                            )
                            // 2
                        )
                        + " secondes"
                    ...
                    self.workout[exo] = (
                        str(
                            (
                                self.recap_exos[exo]["repetitions"][0]
                                + self.recap_exos[exo]["repetitions"][1]
                            )
                            // 2
                        )
                        + " repetitions"
