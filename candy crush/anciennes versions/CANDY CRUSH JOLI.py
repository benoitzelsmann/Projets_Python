...
"""50*50 pixels"""
images2 = { 
    0: os.path.join(chemin_images, 'blanc (1).png'),
    1: os.path.join(chemin_images, 'jaune (1).png'),
    2: os.path.join(chemin_images, 'violet (1).png'),
    3: os.path.join(chemin_images, 'bleu2 (1).png'),
    4: os.path.join(chemin_images, 'vert (1).png'),
    5: os.path.join(chemin_images, 'orange (1).png'),
    6: os.path.join(chemin_images, 'rouge (1).png'),
    7: os.path.join(chemin_images, 'bleu (1).png'),
    }

def images_from_grille(grille):
    taille =  len(grille)
    return [[imageio.imread(images[grille[i][j]]) for j in range(taille)] for i in range(taille)]

def affiche_grille(grille,score,nb,axes):
...
def jeu(nb_coups,size,couleurs,niv):
    ...
    plotid.axis("off")
    axes = [[plt.imshow(liste_images[i][j], extent=[j-0.5, j+0.5, i+0.5, i-0.5],cmap=custom_cmap)for j in range(size+2)]for i in range(size+2)]
    
    """code du jeu répété i fois"""
    i = 0
    while i < nb :
        affiche_grille(grille,score,nb_coups-i,axes)
...
def jeu_final():
    ...
    # coul = int(input("nb de couleurs (de 1 à 7) :"))
