...
class ChessGUI(tk.Tk, Init):
    ...
    def initalisation_board(self):
        ...
        def draw_window():
            ...
            self.legende_adversaire.append(
                self.canvas_3.create_text(70, 20, text=f"Stockfish", font=("Arial", 15), fill="Black"))
            self.legende_adversaire.append(
                self.canvas_3.create_text(160, 20, text=f"(Elo : {self.game.get_niv_elo()})", font=("Arial", 13),
                                          fill="Black"))

            self.advantage_stock = self.canvas_3.create_text(250, 20, text='', font=("Arial", 13, "bold"), fill="black")
            ...
            self.canvas = tk.Canvas(self.frame_board, bg=self.font_color, width=self.canvas_size[0],
                                    height=self.canvas_size[1], highlightbackground=self.font_color)
            ...
            self.canvas_2 = tk.Canvas(self.frame_board, bg=self.font_color, width=self.canvas_2_size[0],
                                      height=self.canvas_2_size[1], highlightbackground=self.font_color)
            self.canvas_2.pack(side=tk.LEFT)

            self.canvas_2.create_rectangle(self.offset_bar,
                                           self.largeur_labels,
                                           self.dimension_bar[0] + self.offset_bar,
                                           self.canvas_2_size[1] - self.largeur_labels,
                                           fill="white")

            self.rectangle_eval = self.canvas_2.create_rectangle(self.offset_bar,
                                                                 self.largeur_labels,
                                                                 self.dimension_bar[0] + self.offset_bar,
                                                                 self.largeur_labels + self.board_size // 2,
                                                                 fill="black")

            self.eval_label = self.canvas_2.create_text(self.offset_bar + self.dimension_bar[0] // 2,
            ...
            self.canvas_4.create_text(100, 15, text=f"Road to GMMM", font=("Arial", 15), fill="Black")
            self.advantage_you = self.canvas_4.create_text(200, 15, text='', font=("Arial", 13, "bold"), fill="black")
        ...
        def path_pieces():
            ...
            self.images = {name: ImageTk.PhotoImage(
                Image.open(f'pieces/{name}.png').resize((int(self.cell_size), int(self.cell_size)))) for name in names}
            self.images_mini = {name: ImageTk.PhotoImage(
                Image.open(f'pieces/{name}.png').resize((int(self.cell_size * 0.5), int(self.cell_size * 0.5)))) for
                name in
                names}
    ...
    def fill_taken_pieces(self):
        ...
        self.pieces_taken["white"].sort(key=lambda x: self.piece_values[x[1]], reverse=True)
        self.pieces_taken["black"].sort(key=lambda x: self.piece_values[x[1]], reverse=True)
        ...
        for i, piece in enumerate(self.pieces_taken["white"]):
            draw = self.canvas_3.create_image((25 + i * 40, 30), anchor=tk.NW, image=self.images_mini[piece])
            ...
            for j, piece in enumerate(liste):
                if piece is not None:
                    index = self.canvas.create_image(
                        (self.largeur_labels + j * self.cell_size, self.largeur_labels + i * self.cell_size),
    ...
    def open_parameters_window(self, event):
        ...
        def update_legende():
            ...
            if self.mode == 'againstcomputer':
                ...
                self.canvas_3.itemconfig(self.legende_adversaire[1], text=f"(Elo : {self.game.get_niv_elo()})")
        ...
        elo_scale = tk.Scale(self.gear_window, from_=100, to=3000, orient="horizontal", length=200,
    ...
    def evaluation_game(self):
        ...
        if eval_win[0] == 'cp':
            ...
            if eval_win[1] != 0:
                ...
                self.proportion = self.board_size / 2 if eval_cp > 0 else -self.board_size / 2
                text = 'M' + str(eval_cp)
            else:
                self.proportion = 0
                if not self.stop:
            ...
            for element in self.coups_possibles.keys():
                ...
                if self.coups_possibles[element] == 0:
                    self.dessins_ronds.append(
                        self.canvas.create_oval(coord_x - self.raduis, coord_y - self.raduis, coord_x + self.raduis,
                                                coord_y + self.raduis, fill='grey30'))
                elif self.coups_possibles[element] == 1:
                    self.dessins_ronds.append(
                        self.canvas.create_oval(coord_x - self.raduis, coord_y - self.raduis, coord_x + self.raduis,
                                                coord_y + self.raduis, fill='red'))
